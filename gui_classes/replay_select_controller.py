...
class ReplaySelectController(DummyWindow):
    ...
    def add_game_replay_handlers(self) -> None:
        _pages = 0
        for _game in range(len(self.saved_games)):
            # only 4 pages XD
            if _game % 4 and _pages != 4:
                _pages += 1
                _game_index = self.saved_games[_game][0]
                _players = self.saved_games[_game][1]
                _text_2_btn = "{}{}{}".format(_game_index, ': ', _players)
                eval("self.ui.game{}_button_{}".format(_game + 1, _pages)).setText(_text_2_btn)
                eval("self.ui.game{}_button_{}".format(_game + 1, _pages)).clicked.connect(
                    lambda state, x=_game_index: self.open_chosen_replay(x)
                )
                eval("self.ui.game{}_button_{}".format(_game + 1, _pages)).setVisible(1)
